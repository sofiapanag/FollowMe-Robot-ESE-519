...
int main(void) {
  ...
  while (1) {
    ...
    /***
     * if the distance is within the desired range i.e 5 - 30 cm
     *  - the robot can turn left, right or move forward
     * if the distance is too small (5 -10), we tell the robot to stop
     * if the distance is really small, then we have to move backwards
     * 
     * **/
    ...
    if ((distance >= 10) && (distance < 30)) {
        if ((IRright == 1) && (IRleft == 1)
        {
            ...
            GoRight();
        } else { // what should we do when both are off?
      ...
      GoBackward();
    } else // distance greater than 30 cm
    {
    }
